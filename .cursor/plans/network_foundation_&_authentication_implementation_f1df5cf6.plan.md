...
4. Add `onRequest` interceptor for automatic header management:
   ...
   - **SSR Header Forwarding (server-side only):**
     - Use `useRequestHeaders(['cookie', 'user-agent', 'accept-language'])` to forward headers
     - Forward only required headers (cookie, user-agent, accept-language for localization)
